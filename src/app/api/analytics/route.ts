...
async function getAnalyticsSummary() {
  const rows = await query('SELECT * FROM analytics_summary');
  return rows;
}

// 获取UTM参数统计
async function getUTMAnalytics() {
  const rows = await query('SELECT * FROM utm_analytics ORDER BY total_clicks DESC');
  return rows;
}
...
async function getPlatformPerformance() {
  const rows = await query('SELECT * FROM platform_performance ORDER BY total_clicks DESC');
  return rows;
}
  ...
  try {
    ...
    try {
      ...
      switch (type) {
        ...
        default:
          // 默认返回汇总数据
          const uniqueVisitorsResult = await query('SELECT COUNT(DISTINCT ip_address) as count FROM click_analytics');
          data = {
